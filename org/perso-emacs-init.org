...
Load the package and default snippets
#+BEGIN_SRC emacs-lisp
  ...
  ;; define a new key binding for company
  (define-key yas-minor-mode-map (kbd "C-<tab>") 'company-yasnippet)

  ;; also add the yasnippet backend as the last
...
Recursively indent all files in a directory with a given extension.
#+BEGIN_SRC emacs-lisp
